...
$testDir = Join-Path (Join-Path (Get-Location) "output") ($timestamp + "--task-8a-exam-questions-runtime-proof")
...
function ChayRuntimeProofCoRetry([string]$command, [string]$logPath, [int]$maxRetry) {
  ...
  throw "runtime proof task8a that bai sau $maxRetry lan thu (exit code cuoi: $exitCode)"
}
...
$runtimeProofExit = ChayRuntimeProofCoRetry "npm run test:task8a:runtime" $runtimeProofLog 3
...
[MUC TIEU TEST]
Kiem chung runtime DB that cho module exam question + answer key task 8A (create/update/delete/permission + cleanup du lieu test).

[LENH DA CHAY]
1) npm run lint
2) npm run typecheck
3) npm run test
4) npm run test:task8a:runtime
...
[GHI CHU]
- Runtime proof script su dung DB dev/test that, tao du lieu test prefix task8a-* va cleanup sau khi chay.
