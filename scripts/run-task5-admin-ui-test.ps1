...
$testDir = Join-Path (Join-Path (Get-Location) "output") ($timestamp + "--task-5-admin-list-and-ui-min")
...
$pngPath = Join-Path $testDir "admin-teacher-verification.png"
$jpgPath = Join-Path $testDir "admin-teacher-verification.jpg"
...
& npm run test *> $testLog
$testExit = $LASTEXITCODE
if ($testExit -ne 0) { throw "Test that bai" }
...
for ($i = 0; $i -lt 100; $i++) {
  ...
  try {
    $response = Invoke-WebRequest -Uri "http://127.0.0.1:3000/admin/teacher-verification" -UseBasicParsing -TimeoutSec 2
...
& $edgePath --headless --disable-gpu --window-size=1440,900 --screenshot=$pngPath http://127.0.0.1:3000/admin/teacher-verification | Out-Null
...
[MUC TIEU TEST]
Kiem tra API list admin-only va UI admin review toi thieu cho task 5.
...
5) Invoke-WebRequest http://127.0.0.1:3000/admin/teacher-verification
...
- HTTP status trang admin review: $httpStatus
- Anh giao dien: admin-teacher-verification.jpg
- Log dev: dev-server.log
- Log loi dev: dev-server.err.log

[GHI CHU]
- Anh chup la trang admin review khi truy cap thuc te tu trinh duyet headless.
- Quyen admin list/review duoc kiem chung boi test route/service trong test.log.
