...
$testDir = Join-Path (Join-Path (Get-Location) "output") ($timestamp + "--task-6a-classroom-runtime-proof")
...
function ChayRuntimeProofCoRetry([string]$command, [string]$logPath, [int]$maxRetry) {
  ...
  throw "runtime proof task6a that bai sau $maxRetry lan thu (exit code cuoi: $exitCode)"
}
...
$runtimeProofExit = ChayRuntimeProofCoRetry "npm run test:task6a:runtime" $runtimeProofLog 3
...
[MUC TIEU TEST]
Kiem chung runtime DB that cho module lop hoc task 6A (create/join/permission + cleanup du lieu test).

[LENH DA CHAY]
1) npm run lint
2) npm run typecheck
3) npm run test
4) npm run test:task6a:runtime
...
[GHI CHU]
- Runtime proof script su dung DB dev/test that, tao du lieu prefix task6a-* va cleanup sau khi chay.
