...
$testDir = Join-Path (Join-Path (Get-Location) 'output') ($timestamp + '--task-3-schema-rls-repository')
New-Item -ItemType Directory -Path $testDir -Force | Out-Null

$lintLog = Join-Path $testDir 'lint.log'
$typeLog = Join-Path $testDir 'typecheck.log'
$unitTestLog = Join-Path $testDir 'test.log'
...
[MUC TIEU TEST]
Kiem tra migration schema, policy RLS, supabase repository, va bo test task 3.
...
- Log chi tiet: lint.log, typecheck.log, test.log, build.log, dev-server.log, dev-server.err.log
