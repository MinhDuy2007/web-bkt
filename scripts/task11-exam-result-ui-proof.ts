...
async function setupProof(statePath: string, pool: pg.Pool): Promise<void> {
  const prefix = `task11-${Date.now()}`;
  const password = "Task11!SafePass123";
  ...
  try {
    ...
    const lop = await taoLopHocBoiGiaoVien(teacherToken, {
      educationLevel: "THPT",
      subjectName: `Lich su ${prefix}`,
      schoolName: "THPT Dev Test",
      gradeLabel: "Khoi 12A1",
      fullClassName: `${prefix}-lop-hoc-ui-result`,
    });
    ...
    const exam = await taoBaiKiemTraTheoLop(teacherToken, {
      classCode: lop.classRecord.classCode,
      title: `Ket qua runtime ${prefix}`,
      description: "Trang ket qua va review attempt toi thieu tren dev/test.",
      status: "published",
    });
    ...
    const q1 = await taoCauHoiChoExam(teacherToken, {
      ...
      promptText: "1945 la nam dien ra su kien nao?",
      points: 2,
      metadataJson: {
        options: ["Tong khoi nghia", "Dien Bien Phu", "Doi moi"],
      },
      answerKey: {
        keyType: "multiple_choice_single",
        correctAnswerText: "Tong khoi nghia",
        correctAnswerJson: {},
    ...
    const q3 = await taoCauHoiChoExam(teacherToken, {
      ...
      promptText: "Viet ngan ve y nghia cua doc lap dan toc.",
      points: 4,
      metadataJson: {
        expectedMinWords: 60,
      },
    ...
    await luuCauTraLoiTheoAttempt(studentToken, {
      ...
      answerText: "Day la bai lam essay can cho cham tay.",
      answerJson: {},
    });
...
async function main(): Promise<void> {
  ...
  if (adapterMode === "mock") {
    throw new Error("Task11 UI proof yeu cau adapter that, khong chap nhan AUTH_ADAPTER_MODE=mock.");
  }
